...
export async function ModuleMessagerie({
  rf, titre, sousTitre, canaux, canalDefaut, avecSujet = false, placeholderVers,
  ...
  return <div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
      ...
      <CarteStat libelle="Livrés (réels)" valeur={s.livres} detail={emisReels > 0 ? `taux ${taux}% sur ${emisReels} émis` : "aucun envoi réel"} couleur="emerald" />
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
      <Carte className="lg:col-span-2">
        ...
        <form action={envoyerMessageAvecPiecesJointes} encType="multipart/form-data" className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div><label className="block text-xs text-slate-400 mb-1">Canal</label><select name="canal" defaultValue={canalDefaut} className={CHAMP}>{canaux.map((c) => <option key={c} value={c}>{c.toUpperCase()}</option>)}</select></div>
          <div><label className="block text-xs text-slate-400 mb-1">Catégorie</label><select name="categorie" className={CHAMP}><option value="transactionnel">Transactionnel</option><option value="marketing">Marketing (soumis DND)</option></select></div>
          <div><label className="block text-xs text-slate-400 mb-1">{labelDe}</label><input name="de" defaultValue={deDefaut} className={CHAMP} /></div>
          <div><label className="block text-xs text-slate-400 mb-1">Destinataire</label><input name="vers" required placeholder={placeholderVers} className={CHAMP} /></div>
          {avecSujet && <div className="md:col-span-2"><label className="block text-xs text-slate-400 mb-1">Sujet</label><input name="sujet" placeholder="Sujet du message" className={CHAMP} /></div>}
          <div className="md:col-span-2"><label className="block text-xs text-slate-400 mb-1">Contenu</label><textarea name="contenu" required rows={3} placeholder="Votre message…" className={CHAMP} /></div>
