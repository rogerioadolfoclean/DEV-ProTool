...
/** Lance un appel AutoDialer IA pour un client sélectionné. */
export async function POST(req: Request) {
  try {
    ...
    const client = await pool.query(
      `SELECT id, name, phone, status FROM clients WHERE id = $1`,
      [clientId],
    );
    ...
    const inserted = await pool.query(
      `INSERT INTO calls (client_id,campaign,status,direction,provider,provider_call_id)
       VALUES ($1,$2,$3,'outbound','twilio',$4) RETURNING id`,
      [c.id, campaign, "initiated", call.sid],
    );
    await pool.query(`UPDATE clients SET last_contact_at = NOW() WHERE id = $1`, [c.id]);

    return NextResponse.json({ ok: true, clientId: c.id, name: c.name, phone: c.phone, callId: inserted.rows[0].id, callSid: call.sid });
  } catch (error) {
